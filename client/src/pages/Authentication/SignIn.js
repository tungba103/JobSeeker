...
function SignIn(props) {
  ...
  const checkSignin = (username, password) => {
    let routes = ['/home', '/candidatehome', '/companyhome', '/admin'];
    let route = '/home';
    for (let account of backendData) {
      if (account.username === username && account.password === password) {
        if (account.role == 'candidate') {
          sessionStorage.setItem('permission', 'candidate');
          route = routes[1];
        }
        if (account.role == 'company') {
          sessionStorage.setItem('permission', 'company');
          route = routes[2];
        } else if (account.role == 'admin') {
          sessionStorage.setItem('permission', 'admin');
          route = routes[3];
        }
        alert('Sign in successfully with ' + sessionStorage.getItem('permission') + ' role');
  ...
  return (
    <div className="flex min-h-full items-center mb-80 justify-center py-12 pt-36 px-4 sm:px-6 lg:px-8">
      <div className="w-full max-w-md space-y-8">
        <div>
          <h2 className="mt-6 text-center text-5xl font-bold tracking-tight text-blue-600">Job Seeker</h2>
        ...
        <div className="mt-8 space-y-6">
          ...
          <div>
            <button
              onClick={() => checkSignin(username, password)}
              type="submit"
            ...
            <p className="flex justify-center text-md  mt-2 hover:underline cursor-pointer text-blue-700" onClick={() => ContinueAsGuest()}>
              Continue as Guest
            </p>
          </div>
        </div>
